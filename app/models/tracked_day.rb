  ...
  def recalculate
    ...
    child.sleep_blocks.covering(for_date).each do |sleep_block|
      if sleep_block.began_before_and_ended_on(for_date)
        self.sleep_total += sleep_block.finish_time - Time.zone.parse(for_date.to_s).beginning_of_day
        ...
        self.sleep_total += (Time.zone.parse(for_date.to_s).end_of_day - sleep_block.start_time).round
